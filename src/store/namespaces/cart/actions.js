...
export const saveCart = async ({state, actions, effects}, data) => {
  try {
    ...
    // data.addItems && actions.alert.showSuccess({message: 'Item added to cart!'});
    ...
    await actions.alert.showError({message: e.response && e.response.errors && e.response.errors.length ? e.response.errors : 'Error', title: 'Saving Cart'});
  }
}
