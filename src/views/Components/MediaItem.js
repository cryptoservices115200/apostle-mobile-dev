...
export const MediaItem = ({title, icon, isBlack, data, chosenMedia, setChosenMedia, ...props}) => {
  const [isPlay, setPlay] = useState(false);
  return (
  <GridItem style={{shadowOffset: {x: 0, y: 10}}}>
    {data?.media?.type === 'IMAGE' ? <Logo source={{uri: data?.media?.source}} style={{width: '100%', position: 'absolute'}}/> : isPlay ? <Video
      source={{uri: data.media?.source}}
      paused={!isPlay}
      style={{width: '100%', height: '100%', position: 'absolute'}}/> :
      <Logo source={{uri: data?.media?.avatar}} style={{width: '100%', position: 'absolute'}}/>
    }
